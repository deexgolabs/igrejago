...
- `/assistente/` — assistente de IA no WhatsApp (Gemini/ChatGPT, chave da própria igreja) e cadastro/atualização coletados por conversa ou por link pessoal: `/assistente/cadastros-pendentes/` (fila de aprovação — nada vira `Person` sem revisão humana), `/assistente/aguardando-atendimento/` (conversas que pediram falar com a secretaria), `/assistente/atualizar-cadastro/<token>/` (público, sem login — link pessoal enviado pelo botão "Enviar link de atualização" em `/pessoas/<id>/`, ou em massa via `/pessoas/campanha/link-atualizacao/`, mesmo filtro/fila da campanha de WhatsApp). Cadastro pendente novo ou pedido de atendimento humano avisa por e-mail (`Church.admin_alert_emails`, mesmo campo do alerta de WhatsApp desconectado) e mostra um contador no menu "Mais". Mídia (foto/áudio/vídeo/documento) recebida pelo assistente não é interpretada — responde só um aviso fixo pedindo texto, nunca chama a IA nem grava como se fosse resposta real.
